...
func (s *Store) GetCustomerAddresses(ctx context.Context, cid int) ([]*Address, error) {
	rows, err := s.db.QueryContext(ctx, "SELECT id, street_line_1, street_line_2, city, state, zip_code, type, is_default,customer_id from addresses WHERE customer_id=$1", cid)
